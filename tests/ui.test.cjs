...
run("details(byId.get('Sun'))");clickText('查看太阳细节');
assert.equal(w.document.getElementById('sheetTitle').textContent,'太阳细节');
assert(text().includes('非实时'));assert(text().includes('Solar System Scope'));assert.equal(viewState.emissive,true);
