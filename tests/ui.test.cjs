...
for(const f of ['astronomy.js',...fs.readdirSync(root).filter(f=>/^stars-\d+\.js$/.test(f)).sort(),'core.js','backup.js','planner.js','app.js'])require('node:vm').runInContext(fs.readFileSync(path.join(root,f),'utf8'),dom.getInternalVMContext());
...
run('fov=85;');click('calibrate');range=body().querySelector('input');range.value='15';range.dispatchEvent(new w.Event('input'));assert.equal(run('fov'),85);clickText('当前设为北');assert.equal(JSON.parse(w.localStorage.getItem('calibration')).az,-90);run('tracking=false; sync();');
